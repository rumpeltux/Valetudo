...
class Viomi extends MiioVacuum {
    ...
    */
    ...
    enableMopMode(enable) {
        // box_type: 0: no box, 1: vacuum box, 2: water tank(?), 3: two-in-one water tank
        let mode = enable
    ...
    checkMopMode() {
        ...
        if (this.box_type == BoxType.VACUUM && mopping_enabled) {
            // vacuum box doesn't support mopping, so disable it.
            this.enableMopMode(false);
            mopping_enabled = false;
            vacuum_enabled = true;
        } else if (this.box_type == BoxType.WATER && vacuum_enabled) {
            // water-only tank doesn't support vacuuming, so set it to mop-only
            // (enableMopMode does this automatically based on the installed box).
            this.enableMopMode(true);
            mopping_enabled = true;
            vacuum_enabled = false;
        }
        if (!this.status.has_mop && mopping_enabled) {
            // Mopping requires a mop, disable it if none is present.
            this.enableMopMode(false);
            mopping_enabled = false;
