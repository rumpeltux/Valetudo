...
class Viomi extends MiioVacuum {
   ...
   async startCleaningZoneByCoords(zones) {
        ...
        for (let k in zones)
        {
            ...
            for (let j = 0; j < zone[4]; j++)
            {
                procesedZones.push([i, 0, x1, y1, x1, y2, x2, y2, x2, y1].join("_"));
                i++;   
            }
        }
        ...
        //syntax is [number of zones][zone rectange][zone rectange][zone rectange][...]
        await this.sendCommand("set_zone", [i].concat(procesedZones), {});
        //start
        await this.startOperation({
            operation: ViomiOperation.START,
            movementMode: ViomiMovementMode.ZONED_CLEAN_OR_MOPPING
